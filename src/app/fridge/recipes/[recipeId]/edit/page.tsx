...
interface Ingredient { id: number; name: string; quantity: string; unit: string; optional: boolean }
interface Step { id: number; description: string }
...
export default function EditRecipePage() {
  ...
  useEffect(() => {
    fetch(`/api/fridge/recipes/${recipeId}`)
      .then((r) => r.json())
      .then((data) => {
        setTitle(data.title ?? '');
        ...
        setIngredients(
          (data.ingredients ?? []).map((i: { name: string; quantity: number; unit: string; optional: boolean }, idx: number) => ({
            id: idx + 1,
            name: i.name,
        ...
        setSteps(
          (data.steps ?? []).map((s: { description: string }, idx: number) => ({
            id: idx + 1,
  ...
  const addIngredient = () =>
    setIngredients((p) => [...p, { id: Date.now(), name: '', quantity: '', unit: '克', optional: false }]);
  const removeIngredient = (id: number) => setIngredients((p) => p.filter((i) => i.id !== id));
  ...
  const addStep = () => setSteps((p) => [...p, { id: Date.now(), description: '' }]);
  const removeStep = (id: number) => setSteps((p) => p.filter((s) => s.id !== id));
  const updateStep = (id: number, description: string) =>
    setSteps((p) => p.map((s) => (s.id === id ? { ...s, description } : s)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/fridge/recipes/${recipeId}`, {
      method: 'PUT',
    ...
    router.push(`/fridge/recipes/${recipeId}`);
    router.refresh();
  };
    ...
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-7">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select value={servings} onChange={(e) => setServings(e.target.value)}
                className="w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm focus:border-neutral-400 focus:outline-none">
                {[1, 2, 3, 4, 5, 6, 8, 10].map((n) => <option key={n} value={n}>{n} 人份</option>)}
              </select>
            </div>
            <div>
        ...
        <section className="rounded-xl border border-neutral-200 bg-white p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addIngredient}
              ...
              <Plus className="h-3.5 w-3.5" /> 新增食材
            </button>
          </div>
          {ingredients.length === 0 && (
            <p className="text-[12px] text-neutral-400">還沒有食材，點擊上方新增</p>
          )}
        ...
        <section className="rounded-xl border border-neutral-200 bg-white p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addStep}
              className="flex items-center gap-1 text-[12px] text-neutral-500 hover:text-neutral-800 transition-colors">
              <Plus className="h-3.5 w-3.5" /> 新增步驟
            </button>
          </div>
          {steps.length === 0 && (
            <p className="text-[12px] text-neutral-400">還沒有步驟，點擊上方新增</p>
          )}
          <div className="space-y-2">
            {steps.map((step, idx) => (
        ...
        <div className="flex gap-3">
          <Link href={`/fridge/recipes/${recipeId}`}
